import torch
from transformers import AutoModel, AutoTokenizer, RobertaTokenizer, PhobertTokenizer
...
from transformers import AdamW, \
    get_linear_schedule_with_warmup, RobertaTokenizer, RobertaModel, RobertaPreTrainedModel
...
class PhoBertaForAE(RobertaPreTrainedModel):
    def __init__(self, config, num_labels=3, model_name='P-SUM'):
        super(PhoBertaForAE, self).__init__(config)
        self.num_labels = num_labels
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None):
        out_puts = self.roberta(input_ids=input_ids,  # token_type_ids=token_type_ids,
...
def train(args):
    ...
    tokenizer = PhobertTokenizer.from_pretrained(modelconfig.MODEL_ARCHIVE_MAP[args.bert_model], add_prefix_space=True)
    ...
    if args.no_valid is False:
        ...
        valid_losses = []
    # <<<<< end of validation declarati on

    model = PhoBertaForAE.from_pretrained(modelconfig.MODEL_ARCHIVE_MAP[args.bert_model], num_labels=len(label_list),
                                         model_name='P-SUM')
    model.to(device)
    # Prepare optimizer
